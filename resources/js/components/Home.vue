<template>
    <div class="bg-gray-100 min-h-screen">
      ...
      <header class="bg-white shadow-md p-4">
        <div class="container mx-auto flex justify-between items-center">
          ...
          <div class="flex items-center space-x-4">
            <div class="cart-icon">
              <router-link :to="route('cart.index')"><i class="fas fa-shopping-cart text-2xl text-gray-700"></i></router-link>
              <span class="badge" id="cartCount">{{ cartCount }}</span>
            </div>
            <router-link v-if="user && user.role === 'customer'" :to="route('customer.points')" class="bg-green-600 text-white px-4 py-2 rounded-full">امتیازات</router-link>
            <router-link v-if="user && user.role === 'seller'" :to="route('seller.dashboard')" class="bg-purple-600 text-white px-4 py-2 rounded-full">داشبورد فروشنده</router-link>
            <router-link v-if="user && user.role === 'courier'" :to="route('courier.dashboard')" class="bg-blue-600 text-white px-4 py-2 rounded-full">داشبورد پیک</router-link>
            ...
            <router-link v-if="user" :to="route('profile')" class="bg-pink-600 text-white px-4 py-2 rounded-full">پروفایل</router-link>
      ...
      <div v-if="showLoginModal" class="modal">
        <div class="modal-content">
          ...
          <button @click="closeLoginModal" class="mt-4 text-gray-600">بستن</button>
        </div>
      </div>
      ...
      <section class="container mx-auto my-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6" id="restaurantList">
          <div v-for="restaurant in filteredRestaurants" :key="restaurant.id" class="restaurant-card bg-white p-4 rounded shadow-md">
            ...
            <router-link :to="route('restaurants.show', restaurant.id)" class="text-pink-600 hover:underline">مشاهده منو</router-link>
          </div>
        </div>
      </section>
        ...
        <div class="container mx-auto">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <ul class="mt-2">
                <li><router-link :to="route('contact')" class="hover:underline">تماس با ما</router-link></li>
                <li><router-link :to="route('faq')" class="hover:underline">سوالات متداول</router-link></li>
                <li><router-link :to="route('terms')" class="hover:underline">قوانین و مقررات</router-link></li>
              </ul>
            </div>
            <div>
  ...
  export default {
    ...
    methods: {
      route, // برای استفاده مستقیم از route در تمپلیت
      openLoginModal() {
      ...
      async fetchRestaurants() {
        try {
          const response = await axios.get(route('api.restaurants.index'));
      ...
      async fetchCartCount() {
        ...
        if (this.user && token) {
          try {
            const response = await axios.get(route('api.cart.index'), {
              headers: { 'Authorization': `Bearer ${token}` },
            });
