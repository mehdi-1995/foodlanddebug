...
<head>
    ...
    <title>افزودن آیتم منو - {{ $restaurant->name }}</title>
...
<body class="bg-gray-100 font-sans">
    ...
    <header class="bg-white shadow">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center">
            <h1 class="text-2xl font-bold text-gray-800">افزودن آیتم منو - {{ $restaurant->name }}</h1>
    ...
    <main class="container mx-auto px-4 py-6">
        <div class="bg-white rounded-lg shadow-md p-4">
            <h2 class="text-xl font-semibold mb-4">افزودن آیتم جدید</h2>
            <form action="{{ route('seller.menu.store') }}" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="name" class="block text-gray-700">نام</label>
                    <input type="text" name="name" id="name" class="w-full p-2 border rounded-lg" value="{{ old('name') }}">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="price" id="price" class="w-full p-2 border rounded-lg" value="{{ old('price') }}">
                    @error('price') <p class="text-red-600">{{ $message }}</p> @enderror
                </div>
                <div class="mb-4">
                    <label for="category" class="block text-gray-700">دسته‌بندی</label>
                    <input type="text" name="category" id="category" class="w-full p-2 border rounded-lg" value="{{ old('category') }}">
                    @error('category') <p class="text-red-600">{{ $message }}</p> @enderror
                </div>
                <div class="mb-4">
                    <label for="image" class="block text-gray-700">لینک تصویر</label>
                    <input type="url" name="image" id="image" class="w-full p-2 border rounded-lg" value="{{ old('image') }}">
                    @error('image') <p class="text-red-600">{{ $message }}</p> @enderror
                </div>
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg">افزودن</button>
            </form>
        </div>
    </main>
